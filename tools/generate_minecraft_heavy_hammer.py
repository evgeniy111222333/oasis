...
def runtime_y(value: float) -> float:
    return RUNTIME_GRIP_Y + (value - MAIN_GRIP_Y) * RUNTIME_Y_SCALE


def preserved_scale(value: float) -> list[float]:
    return [value, value / RUNTIME_Y_SCALE, value]


def validate_minecraft_model(model: dict) -> None:
    for element in model["elements"]:
        for key in ("from", "to"):
            for axis, coordinate in zip("XYZ", element[key]):
                if not -16.0 <= coordinate <= 32.0:
                    raise ValueError(
                        f"{element['name']} {key}.{axis}={coordinate:.4f} выходит за пределы Minecraft [-16, 32]"
                    )

    if not math.isclose(runtime_y(MAIN_GRIP_Y), RUNTIME_GRIP_Y, abs_tol=1.0e-9):
        raise ValueError("Нижний хват должен оставаться в центре ItemTransform Y=8")
...
def minecraft_model(parts: list[Cube]) -> dict:
    model = {
        ...
        "elements": [
            {"name": part.name,
             "from": [part.start[0], runtime_y(part.start[1]), part.start[2]],
             "to": [part.end[0], runtime_y(part.end[1]), part.end[2]],
        ...
        "display": {
            ...
            "thirdperson_righthand": {"rotation": [0, 0, 0], "translation": [0, 0, 0], "scale": preserved_scale(0.57)},
            "thirdperson_lefthand": {"rotation": [0, 0, 0], "translation": [0, 0, 0], "scale": preserved_scale(0.57)},
            "firstperson_righthand": {"rotation": [0, -90, 25], "translation": [1.13, 3.2, 1.13], "scale": preserved_scale(0.61)},
            "firstperson_lefthand": {"rotation": [0, 90, -25], "translation": [1.13, 3.2, 1.13], "scale": preserved_scale(0.61)},
            "ground": {"rotation": [0, 0, 78], "translation": [0, 2.4, 0], "scale": preserved_scale(0.34)},
            "gui": {"rotation": [25, 142, -32], "translation": [0, -0.5, 0], "scale": preserved_scale(0.43)},
            "fixed": {"rotation": [0, 0, -45], "translation": [0, -1.0, 0], "scale": preserved_scale(0.40)},
        },
    }
    validate_minecraft_model(model)
    return model
...
def main() -> None:
    ...
    manifest = {"style": "Утилитарный средневековый инструмент в стилистике Minecraft", "texture": [SIZE, SIZE],
                ...
                "runtimeGripY": RUNTIME_GRIP_Y, "runtimeYScale": RUNTIME_Y_SCALE,
                "runtimeMaxY": RUNTIME_MAX_Y, "bounds": bounds,
