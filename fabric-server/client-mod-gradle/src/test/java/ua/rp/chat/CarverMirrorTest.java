...
public final class CarverMirrorTest {
    ...
    private static void verifyCameraMath() {
        ...
        // Work-camera montage: shots stay inside the orbit cone, never repeat, and the cooldown
        // sits in the intended sharp-cut window.
        require(CarverCameraMath.workShotCount() >= 6,
                "Work montage must offer several distinct shots");
        for (int index = 0; index < CarverCameraMath.workShotCount(); index++) {
            CarverCameraMath.WorkShot shot = CarverCameraMath.workShot(index);
            require(Math.abs(shot.yawDeltaDeg()) >= 90.0f,
                    "Every shot must swing to a clearly different side");
            require(shot.pitchDeg() >= CarverCameraMath.MIN_PITCH
                            && shot.pitchDeg() <= CarverCameraMath.MAX_PITCH,
                    "Shot pitch must stay inside the orbit cone");
            require(shot.dist() >= CarverCameraMath.MIN_DIST
                            && shot.dist() <= CarverCameraMath.MAX_DIST,
                    "Shot distance must stay inside the zoom range");
        }
        java.util.Random random = new java.util.Random(7L);
        int previous = -1;
        for (int i = 0; i < 60; i++) {
            int next = CarverCameraMath.nextWorkShot(random, previous);
            require(next != previous && next >= 0 && next < CarverCameraMath.workShotCount(),
                    "The montage must never repeat the previous shot");
            previous = next;
        }
        for (int i = 0; i < 40; i++) {
            int cooldown = CarverCameraMath.nextCutCooldown(random);
            require(cooldown >= CarverCameraMath.WORK_CUT_MIN_TICKS
                            && cooldown <= CarverCameraMath.WORK_CUT_MAX_TICKS,
                    "Cut cooldown must sit in the sharp-cut window");
        }
        require(CarverCameraMath.smoothStep(0.0) == 0.0
                        && CarverCameraMath.smoothStep(1.0) == 1.0
                        && CarverCameraMath.smoothStep(0.5) == 0.5,
                "Cut ease must pin its endpoints and midpoint");
        System.out.println("CarverWorkMontageTest: shots, cooldown and ease passed");
    }
