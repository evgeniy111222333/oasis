...
public final class CarverTest {
    ...
    private static void verifyEvaluation() {
        ...
        for (int x = 0; x < 16; x++) {
            for (int z = 0; z < 16; z++) slab.set(DraftMask.index(x, 8, z));
        }
        ua.rp.chat.microvoxel.MicrovoxelVolume afterSlab = full.copy();
        for (int cell : slab.cells()) afterSlab.remove(cell);
        require(Math.abs(CarverEvaluation.stability(afterSlab) - 0.5) < 0.05,
                "A middle-plane cut must halve the largest component");

        DraftMask a = new DraftMask();
        DraftMask b = new DraftMask();
        for (int i = 0; i < 10; i++) {
            a.set(DraftMask.index(i, 0, 0));
            b.set(DraftMask.index(i, 0, 0));
        }
        require(CarverEvaluation.iou(a, b) == 1.0, "Identical masks must have IoU 1");
